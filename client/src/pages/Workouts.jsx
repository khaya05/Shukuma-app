...
export default function Workouts() {
  ...
  const fetchRandomExercise = async () => {
    setLoading(true);
    try {
      const response = await customFetch.get('/api/v1/exercises/random');
  ...
  const handleComplete = async () => {
    try {
      if (!exercise?._id) {
        toastService.error('Exercise not loaded');
        return;
      }

      await customFetch.post('/api/v1/workouts', {
    ...
    <div className='flex min-h-screen bg-gray-100'>
      <main className='flex-1 overflow-y-auto px-8 flex flex-col items-center justify-start pt-6'>
        <div className='w-full max-w-2xl mb-8 bg-white bg-opacity-10 backdrop-blur-md rounded-lg p-6 flex justify-between items-center'>
          ...
          <h2 className='text-2xl font-bold text-white'>Let's Get Moving!</h2>
        ...
        <div className='m-4 text-center text-white text-lg font-semibold'>
